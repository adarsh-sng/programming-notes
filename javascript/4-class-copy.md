```js
const a = { name: "Adarsh", age: 22, address: { city: "Delhi", country: "India" } };
...
Person.prototype.greet = function() { return `Hi, I'm ${this.name}`; }; // SHARED
...
function Person(name) {
  this.name = name;
  this.greet = function() { return `Hi ${this.name}`; }; // NEW FUNCTION EACH TIME
}

// Class - all instances share the same greet method
class Person {
  constructor(name) { this.name = name; }
  greet() { return `Hi ${this.name}`; } // SHARED ON PROTOTYPE
}
